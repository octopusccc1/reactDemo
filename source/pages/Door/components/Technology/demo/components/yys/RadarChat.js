...
class D3RadarLineChart extends React.Component {
  componentDidMount() {
    ...
    let g = chart.append("g").attr("transform", "translate(" + containerWidth / 2 + "," + (height + margin.top + margin.bottom) / 2 + ")");

    const names = { attack: '攻击', life: '生命', defence: '防御', speed: '速度', crit: '暴击' };
    ...
    //const maxSerieValue = d3.max(series, function (s) { return d3.max(s, function (d) { return d.value; }); });
    ...
    // let tick = g.selectAll(".tick") // 绘画所有轴线条  
    //   .data(keys.map(key => {
    //     return [
    //       { angle: 0, radius: 0 },
    ...
    let serie = g.selectAll(".serie") // 绘画雷达线条  
      .data(series)
      .enter().append("g")
      .attr("class", "serie")
    ...
    serie.append("path") // 开始绘画雷达线条      
      .attr("class", "radar-line")
      .style("stroke", function (d) { return z(d[0].className); })
      .attr('fill', function (d) { return z(d[0].className); })
    ...
    let legend = chart.append("g") // 画legend
...
D3RadarLineChart.propTypes = {
  data: PropTypes.arrayOf(PropTypes.shape({
    ...
    defence: PropTypes.string.isRequired,
