...
class D3BrokenLine extends React.Component {
	componentDidMount() {
		...
		const series = d3.map(data[0]).keys().slice(1).map(function (key) { // 生成两条线的数组
		...
		let x = d3.scaleTime() // 定义x轴
			.domain([d3.timeParse("%Y")(data[0].date), d3.timeParse("%Y")(data[data.length - 1].date)])
			.range([0, width]);

		let y = d3.scaleLinear() // 定义y轴
		...
		label.append("text") // 生成数值文字
			.attr("dy", ".35em")
			.attr('fill', '#000')
			.text(function (d) { return d.value + ' 吨'; })
			.filter(function (d, i) { return i === data.length - 1; })
			.append("tspan")　// 生成名文字
			.attr("class", "label-key")
			.attr('fill', '#000')
			.attr('font-size', '14px')
			.text(function (d) { return " " + names[d.key]; });
	...
	render() {
		return (
			<div className="line-chart--simple">
				<svg ref={node => this.chartRef = node}></svg>
			</div>
		)
	}
}
D3BrokenLine.propTypes = {
